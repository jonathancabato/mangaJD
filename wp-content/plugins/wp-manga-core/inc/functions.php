<?php
	...
	class WP_MANGA_FUNCTIONS {
		...
		function manga_output_rating( $rate, $is_user_rating = false ) {
			...
			if ( $is_user_rating ) {
				$html .= '<div class="post-rating-score-digit"><span class="score font-meta total_votes">' . 'Ratings</span>'. '<p>' .$rate.'</p></div>' ;
			} else {
				$html .= '<div class="post-rating-score-digit"><span class="score font-meta total_votes">' . 'Ratings</span>'. '<p>' .$rate .'</p></div>' ;
			}
			...
			if ( 1 == strlen( $rate ) ) {
				for ( $i = 0; $i < $max_rate; $i ++ ) {
					if ( $i < $rate ) {
						...
						$html .= '<i class="ion-ios-star ratings_stars"></i>';
					}
				}
			} else {
		...
		function print_ranking_rank( $manga_id ) {
			
			$rank        = $this->get_manga_rank( $manga_id );
		
			
			echo apply_filters('madara_manga_ranking_views', sprintf( _n( ' %1s ', ' %1s ', $views, WP_MANGA_TEXTDOMAIN ), $rank, $views ), $manga_id, $rank, $views);
		}
		...
		function print_ranking_views( $manga_id ) {
			...
			echo apply_filters('madara_manga_ranking_views', sprintf( _n( ' %1s%2s ', ' %1s%2s ', $views, WP_MANGA_TEXTDOMAIN ), $rank, $views ), $manga_id, $rank, $views);
		}
		...
		function create_bookmark_link( $post_id = '', $is_manga_single = '', $chapter_slug = '' ) {
				...
				if ( ! empty( $bookmark_manga ) ) {
					...
					if ( $index !== false ) {
						...
						if(!$wp_manga_functions->is_manga_single()){
							...
							$output .= '<a href="javascript:void(0)" class="btn-primary wp-manga-delete-bookmark madara_bookmarked" data-action="delete-bookmark" data-post-id="' . $post_id . '" title="' . esc_attr__('',WP_MANGA_TEXTDOMAIN) . '"><i class="icon ion-md-checkmark"></i></a>';
							
						}else{
							$output .= '
								
							<div class="button-wrapper">
								<a href="javascript:void(0)" class="btn-primary wp-manga-delete-bookmark madara_bookmarked" data-action="delete-bookmark" data-post-id="' . $post_id . '" title="' . esc_attr__('',WP_MANGA_TEXTDOMAIN) . '">
									<i class="icon ion-md-checkmark"></i> <span class="bookmark-text">Add to Subscribe </span>
								</a>
							</div>
							...
							// 	$output .= '<span>' . $total_bookmarked > 2 ? sprintf(esc_attr__( 'You have already added this with %s others', WP_MANGA_TEXTDOMAIN ), wp_manga_number_format_short($total_bookmarked)) : ($total_bookmarked == 2 ? esc_attr__( 'You have already added this with one another', WP_MANGA_TEXTDOMAIN ) : esc_attr__( 'You have already added this', WP_MANGA_TEXTDOMAIN )) . '</span>';
							// 	$output .= '</div>';
							// }
			...
			if( !$wp_manga_functions->is_manga_single()){
				$output .= '<a class = "wp-manga-action-button" href="#" data-action="bookmark" data-post="' . $post_id . '" data-chapter="' . $chapter_id . '" data-page="' . $page . '" title="' . esc_attr__('Bookmark', WP_MANGA_TEXTDOMAIN) . '"><i class="fas fa-bookmark"></i></a>';
			}else{
			$output .= '
			<div class="button-wrapper">
				<a href="#" class="btn-primary wp-manga-action-button" data-action="bookmark" data-post="' . $post_id . '" data-chapter="' . $chapter_id . '" data-page="' . $page . '" title="' . esc_attr__('Bookmark', WP_MANGA_TEXTDOMAIN) . '"><i class="fas fa-plus"></i><span class="bookmark-text">Add to Subscribe </span></a></div>';
			}
