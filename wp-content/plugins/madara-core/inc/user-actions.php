<?php
	...
	class WP_MANGA_USER_ACTION {
		...
		function get_user_section( $size = 50, $echo = false ) {
			...
			$link_settings = $this->get_user_tab_url( 'account-settings' );
			$link_bookmark = $this->get_user_tab_url( 'bookmark' );
			$link_reader = $this->get_user_tab_url( 'reader-settings' );
			$user_id = get_current_user_id(); 
 	 		$user_info = get_userdata($user_id);
  			$mailadresje = $user_info->user_email;
  			$username = $user_info->display_name;
			$balance = do_shortcode("[mycred_my_balance wrapper=0 title_el='' balance_el='']"); 
          	$icon = get_avatar( $current_user->ID, $size );
			$user_menu = '';
			...
			if ( ! empty( $link ) ) {
				$user_menu .= '
					<li class="user">
						<a href="' . esc_url( $link_settings ) . '"><i>'
						.$icon.
						'</i><div class="user-wrap">
						<p>' . esc_html__( $username, WP_MANGA_TEXTDOMAIN ) . '</p> <p class="email">'.esc_html__( $mailadresje, WP_MANGA_TEXTDOMAIN ) .'</p> </div></a>
						
					</li>
					<ul class="icon-list"><li class="points width-33"> 
						<a href="' . esc_url( $link ) . '"><i class="fas fa-star"></i><p>' . esc_html__( $balance . ' Points', WP_MANGA_TEXTDOMAIN ) . '</p></a>
					</li>
					<li><a href="javascript:void(0)" data-toggle="modal" data-target="#myCredForm" class="btn"><i class="fas fa-coins"></i>Buy</a></li>' .

				// 	<li class="light-mode  width-33">
					
                //    <a href="#" class="wp-manga-action-button" data-action="toggle-contrast">
                //  <i class="fas fa-toggle-on"></i>  <p>Light Mode</p></a>
				// 	</li>
					
					'<li class="bookmark  width-33">
						<a href="' . esc_url( $link_bookmark ) . '"><i class="fas fa-book"></i><p>' . esc_html__( 'Bookmarks', WP_MANGA_TEXTDOMAIN ) . '</p></a>
					</li>
					</ul>
				 	<li>
						<a href="' . esc_url( $link ) . '">' . esc_html__( 'Profile', WP_MANGA_TEXTDOMAIN ) . '</a>
					</li>
                     <li>
					 	<a href="' . esc_url( $link ) . '">' . esc_html__( 'Dashboard', WP_MANGA_TEXTDOMAIN ) . '</a>
					 </li>
					<li>'.apply_filters('wp_manga_user_menu_before_items', '').'</li>
                    <li>
						<a href="' . esc_url( $link_reader ) . '">' . esc_html__( 'Settings', WP_MANGA_TEXTDOMAIN ) . '</a>
					</li>';
			}
