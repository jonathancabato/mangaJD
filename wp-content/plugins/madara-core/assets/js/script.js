	...
	$(document).on( 'click', '.wp-manga-action-button', function(e) {
		...
		if($(this).attr('data-action') == 'bookmark') {
			...
			if( $('.add-bookmark').length != 0 ) {
				...
				if($('.action_list_icon li').length > 2){
					$('.action_list_icon > li:nth-child(3)').css('opacity', '0.5');
				}else{
				...
				jQuery.ajax({
					...
					success: function( response ) {
						if ( response.success ) {
							if( $('.add-bookmark').length != 0 ) {
								var html = "";
								html += '<div class="action_icon"><div class="button-wrapper">'
								html += '<a href="javascript:void(0)" class="btn-primary wp-manga-delete-bookmark madara_bookmarked" data-action="delete-bookmark" data-post-id="' + postID + '">'
								html +=	'<span class="overlay-btn"></span>'
								html += '<span class="overlay-btn-text"><i class="icon ion-md-checkmark"></i><span class="bookmark-text">Add to Subscribe </span></span>'
								html +=	'</a>'
								html += '</div></div>'
								$('.add-bookmark').empty();
								$('.add-bookmark').append( html );
								$('.add-bookmark').css('opacity', '1');

							}else{
								if($('.action_list_icon li').length > 2){
									$('.action_list_icon > li:nth-child(3)').empty();
									$('.action_list_icon > li:nth-child(3)').append( response.data );
									$('.action_list_icon > li:nth-child(3)').css('opacity', '1');
								}else{
		        ...
		        success: function( response ) {
		            if ( response.success ) {
						...
						if( typeof isMangaSingle !== 'undefined' && isMangaSingle == true ) {
							var html = "";
							html += '<div class="action_icon"> <div class="button-wrapper">'
							html += '<a href="#" class="btn-primary wp-manga-action-button" data-action="bookmark" data-post="'  + postID + '" data-chapter data-page="' + page +'" >'
							html +=	'<span class="overlay-btn"></span>'
							html += '<span class="overlay-btn-text"><i class="fas fa-plus"></i> <span class="bookmark-text">Add to Subscribe</span> </span>'
							html +=	'</a>'
							html += '</div></div>'
							$('.add-bookmark').empty();
							$('.add-bookmark').append( html );
