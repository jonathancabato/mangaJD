...
class TextField extends Field {
   ...
   public function frontFormHtml($name, $args, $options, $currentUser, $uniqueId, $isMainForm) {
		...
		$manga_id = get_queried_object_id();
		$chapter_id = 0;
		if ( $cur_chap != '' ) {
			global $wp_manga_functions;
			$chapter = $wp_manga_functions->get_chapter_by_slug( $manga_id, $cur_chap );
			
			$chapter_id = $chapter ? $chapter['chapter_id'] : 0;
		}
        ?>
        <div class="wpdiscuz-item <?php echo $name, '-wrapper', ($hasIcon ? ' wpd-has-icon' : ''), ($hasDesc ? ' wpd-has-desc' : ''); ?>">
            ...
            <?php } ?>
            <?php $required = $args['required'] ? 'required="required"' : ''; ?>
            <input <?php echo $required; ?> class="<?php echo $name; ?> wpd-field wpd-field-text" type="text" name="<?php echo $name; ?>" value="<?php echo ($name == "chapter_id" && $chapter_id) ? $chapter_id : ''; ?>" placeholder="<?php _e($args['name'], 'wpdiscuz');
